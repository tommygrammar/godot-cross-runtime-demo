...
public:
  ...
  void _frame_update() { process_api_commands(); }

  CommandProcessor() { singleton = this; }

  ~CommandProcessor() { singleton = nullptr; }
};
...
void register_command_processing() {
  ...
  if (!tree) {
    MessageQueue::get_singleton()->push_callable(
        callable_mp_static(&register_command_processing));
    return;
  }
  ...
  Callable update_callable = callable_mp(CommandProcessor::get_singleton(),
                                         &CommandProcessor::_frame_update);
...
void unregister_command_processing() {
  ...
  if (cp) {
    SceneTree *tree = SceneTree::get_singleton();
    if (tree) {
      tree->disconnect("process_frame",
                       callable_mp(cp, &CommandProcessor::_frame_update));
    }
    memdelete(cp);
  }
}
